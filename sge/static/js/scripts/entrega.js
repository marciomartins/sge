...
$('#bt-adicionar-linha-cliente').on('click', function(){
    ...
    var total = parseInt($('#id_entregacliente_set-TOTAL_FORMS').val(), 10);
    var new_order_num = parseInt($('#tb-clientes > tbody tr:last :text[name ^= entregacliente_set-]:first').attr('name').split('-')[1], 10)+1;
    ...
    $('#id_entregacliente_set-TOTAL_FORMS').val(total+1);
    estilizar_linhas();
});
function reordenar_campos_cliente() {
    ...
    $('#tb-clientes > tbody tr').each(function(i) {
        $(this).find('[id ^= id_entregacliente_set-]').each(function(){
...
$('#bt-confirm').on('click', function(e){
    ...
    if (total === 1) {
        ...
        total = parseInt($('#id_entregacliente_set-TOTAL_FORMS').val(), 10);
    }
...
/*
 * Mostra o valor para cada cliente
 ******************************************************************************/
/*
$('#clientes select[name $= -cliente]').on('change', function(){
    var label = $(this).closest('tr').find('span.valor-entrega-cliente');
    var field = $(this).closest('tr').find(':hidden.valor-entrega-cliente-hd');
    var cliente_id = $(this).val();
    var entregador_id = $(this).val();

    if ($(this).val()) {
        $.get('/entregador-cliente/valor', {'cliente_id': cliente_id, 'entregador_id': entregador_id}, function(data) {
            label.html('Valor: R$ ' + data.object.valor_maximo_entrega).fadeIn().removeClass('label-important').addClass('label-info');
...
$('.no-action').on('click', function(){
    location.href = '/entrega/consulta';
});
...
$('.cancel').on('click', function(){
    ...
    var entrega_saida = $(this).parent().find('.entrega_saida').text();
    $('#confirm-cancel-entrega .modal-body')
        .html('<p>Você tem certeza que deseja cancelar a entrega selecionada?</p> <p>Entrega de: <strong>'+entregador_nome+'</strong> Enviada em: <strong>'+entrega_saida+'</strong></p>');
